...
extension Collection.Remove {
    ...
    public protocol Last: Collection.`Protocol` & ~Copyable {
        ...
        ///
        /// Static implementation primitive mirroring `Swift.Array.removeLast()` /
        /// `Swift.RangeReplaceableCollection.removeLast()` — preserves drop-in
        /// vocabulary alignment with the stdlib. Use `.remove.last()` at call
        /// sites for the fluent API; conformers may also expose
        /// `Swift.Array.removeLast()` instance vocabulary additively.
        static func removeLast(_ base: inout Self) -> Element?
    }
}
